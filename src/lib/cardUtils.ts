...
// deck). Not used for comparison: see compareCards below, which never
// breaks same-rank ties by suit.
...
// card exchange). Same-rank cards are a genuine tie (0) regardless of suit —
// RULES.md "Card Ranking" never distinguishes by suit, including among level
// cards: hearts' wild-card status (RULES.md "Level Cards & Wild Cards")
// changes what a level-rank heart can represent, not its rank against other
// level cards.
export function compareCards(a: Card, b: Card, levelRank: StandardRank): number {
  return getCardRank(a, levelRank) - getCardRank(b, levelRank);
}
