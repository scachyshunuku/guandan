...
// is_connected = false after 30s." Used both to derive a participant's live
// connected status at read time (gameStateResponse.ts - self-healing even if
// no heartbeat has swept the DB recently) and by the heartbeat route's own
// sweep of every *other* participant's stored `is_connected`.
