...
4. **`open-pr`** (skill) — opens the PR with a summary, verification results, screenshots, review order, and inline annotations. Once `verify-task` has passed, this step (including `git push` and `gh pr create`) is pre-authorized — don't stop to ask for confirmation before pushing or opening the PR.
